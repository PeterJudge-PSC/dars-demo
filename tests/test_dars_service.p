/*------------------------------------------------------------------------
    File        : test_list_deliveries.p
    Purpose     : 

    Syntax      :

    Description : 

    Author(s)   : 
    Created     : Mon Aug 04 11:18:20 EDT 2014
    Notes       :
  ----------------------------------------------------------------------*/

/* ***************************  Definitions  ************************** */

block-level on error undo, throw.

using TripleDee.Shopping.ShoppingService.
using Progress.Json.ObjectModel.JsonObject.
using TripleDee.Shopping.DeliveryEntity.
using TripleDee.Shopping.OrderEntity.
using TripleDee.Shopping.OrderStatusEnum.
using TripleDee.Shopping.BusinessEntity.

{TripleDee/Shopping/delivery_dataset.i}
{TripleDee/Shopping/order_dataset.i}

/* ***************************  Main Block  *************************** */

define variable oShopSvc as ShoppingService no-undo.
define variable oDeliverySvc as DeliveryEntity no-undo.
define variable oOrderSvc as OrderEntity no-undo.
define variable cDriver as character no-undo.
define variable cDelivery as character no-undo.
define variable cOrder as character no-undo.
define variable cReturnCode as character no-undo.
define variable cReturnReason as character no-undo.
define variable cStatus as character no-undo.
define variable cPOD as longchar no-undo
    init ''.

oShopSvc = new ShoppingService().
oDeliverySvc = new DeliveryEntity().
oOrderSvc = new OrderEntity().

/* list orders */
oOrderSvc:ListOrders(new JsonObject(), output dataset dsOrder).
dataset dsOrder:write-json('file', 'temp/orders-001.json', true).

/* open delivery */
cDriver = 'DRV105'.
cDelivery = oShopSvc:OpenDelivery(cDriver).

/* list deliveries */
oDeliverySvc:GetDeliveryByCode(cDelivery, output dataset dsDriverDelivery).
dataset dsDriverDelivery:write-json('file', 'temp/delivery-001.json', true).


/* schedule pickup */
find first Order where
           Order.OrderStatus eq OrderStatusEnum:Delivered:Name
           no-lock no-error.
if available Order then
do:           
    cOrder = Order.Code.
    
    /* request return */
    cReturnReason = 'too big'.
    cStatus = oOrderSvc:RequestReturn(input        cOrder,
                                      input-output cReturnReason,
                                            output cReturnCode).
    
    if cStatus eq OrderStatusEnum:ReturnApproved:Name then
    do:
        oShopSvc:SchedulePickup(cOrder, output cDriver).
        oDeliverySvc:NotifyDriver(cOrder, cReturnCode, cDriver).
    end.
end.

/* list deliveries */
oDeliverySvc:GetDeliveryByCode(cDelivery, output dataset dsDriverDelivery).
dataset dsDriverDelivery:write-json('file', 'temp/delivery-002.json', true).

def var ofilter as JsonObject.

ofilter = new JsonObject().
ofilter:Add('code', cDelivery).

oDeliverySvc:ListDriverDeliveries(input ofilter, output dataset dsDriverDelivery).
dataset dsDriverDelivery:write-json('file', 'temp/delivery-005.json', true).


/* perform the delivery */
BusinessEntity:EnableDatasetForUpdate(dataset dsDriverDelivery:handle).
find first eDelivery.
for each eDeliveryItem:
    case eDeliveryItem.DeliveryStatus:
        when 'ReturnApproved' then cStatus = 'ReturnReceived'.
        when 'Ordered'        then cStatus = 'Delivered'.
    end case.
    assign eDeliveryItem.DeliveredAt = add-interval(eDelivery.StartTime,
                                                    random(10, 5 * 60),
                                                    'minutes')
           eDeliveryItem.DeliveryStatus = cStatus
           eDeliveryItem.ProofOfDelivery = cPOD. 
end.

BusinessEntity:DisableDatasetForUpdate(dataset dsDriverDelivery:handle).

oDeliverySvc:UpdateDriverDeliveries(input-output dataset dsDriverDelivery by-reference).
dataset dsDriverDelivery:write-json('file', 'temp/delivery-003.json', true).

/* close delivery */
oShopSvc:CloseDelivery(cDelivery).

oDeliverySvc:GetDeliveryByCode(cDelivery, output dataset dsDriverDelivery).
dataset dsDriverDelivery:write-json('file', 'temp/delivery-004.json', true).

/* list orders */
oOrderSvc:ListOrders(new JsonObject(), output dataset dsOrder).
dataset dsOrder:write-json('file', 'temp/orders-002.json', true).

catch e as Progress.Lang.Error :
    message 
        e:GetMessage(1) skip(2)
        e:CallStack
    view-as alert-box.
		
end catch.

finally:
    message 
    'DARS done for delivery ' cDelivery
    view-as alert-box.
end finally.